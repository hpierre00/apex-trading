...
const APPROVAL_THRESHOLD = 0.20; // 20% change requires admin approval

export async function handler() {
  // 1. Aggregate accuracy per condition_key from last 90 days
  ...
  if (error) {
    console.error('fetch outcomes error', error.message);
  ...
  // 2. For each condition, compute new weight and decide auto vs. pending
  for (const [key, stats] of Object.entries(groups)) {
    if (stats.total < 10) continue; // minimum sample size

    const accuracy  = stats.correct / stats.total;
    const newWeight = Math.max(0.1, Math.min(3.0, accuracy / 0.5)); // baseline = 0.5
    ...
    if (changePct < APPROVAL_THRESHOLD) {
      ...
      // Queue for admin approval — only update pending columns, do not touch live weight
      await supabase.from('learned_weights').upsert({
        condition_key:       key,
  ...
  // 3. Write daily agent performance summary
  await computeAndStoreAgentPerformance();

  return {
    statusCode: 200,
    body: `weights updated: ${autoApplied} auto-applied, ${queued} queued for approval`
  };
}
...
async function computeAndStoreAgentPerformance() {
  ...
  for (const agent of agents) {
    ...
    // Best/worst condition by accuracy
    const condMap = {};
    for (const r of data) {
