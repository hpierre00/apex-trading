...
exports.handler = async (event) => {
  ...
  const fetchRes = await fetch(fetchUrl, { headers: SB_HEADERS });
  if (!fetchRes.ok) {
  ...
  for (const sig of signals) {
    try {
      ...
      const polyUrl  = `https://api.polygon.io/v2/aggs/ticker/${sig.symbol}/range/${multiplier}/${timespan}/${fromDate}/${toDate}` +
                       `?apiKey=${POLYGON_API_KEY}&limit=50&sort=asc&adjusted=true`;

      const polyRes = await fetch(polyUrl);
      if (!polyRes.ok) {
        console.warn(`[evaluate] Polygon ${sig.symbol} returned ${polyRes.status}`);
        await markEvaluated(sig.id, { is_evaluated: true, outcome_evaluated_at: nowISO });
        continue;
      }
      ...
      // direction comes from signal_direction column (DB name) or direction column
      const rawDirection = sig.signal_direction || sig.direction || 'BUY';
      const direction = rawDirection === 'BUY' ? 1 : -1;
...
async function markEvaluated(id, fields) {
